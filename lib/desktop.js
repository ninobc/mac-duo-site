...
export function buildDesktop(image, W, H, scale) {
  ...
  // Padded canvas with stretched edges.
  ...
  p.drawImage(desk, 0, 0, 1, desk.height, 0, ip, ip, desk.height);                                  // left
  p.drawImage(desk, desk.width - 1, 0, 1, desk.height, ip + desk.width, ip, ip, desk.height);       // right
  p.drawImage(desk, 0, 0, desk.width, 1, ip, 0, desk.width, ip);                                    // top
  p.drawImage(desk, 0, desk.height - 1, desk.width, 1, ip, ip + desk.height, desk.width, ip);       // bottom
  p.drawImage(desk, 0, 0, 1, 1, 0, 0, ip, ip);
  p.drawImage(desk, desk.width - 1, 0, 1, 1, ip + desk.width, 0, ip, ip);
  p.drawImage(desk, 0, desk.height - 1, 1, 1, 0, ip + desk.height, ip, ip);
  p.drawImage(desk, desk.width - 1, desk.height - 1, 1, 1, ip + desk.width, ip + desk.height, ip, ip);
  return padded;
}
