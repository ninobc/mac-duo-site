...
export const fragmentShader = /* glsl */`
  ...
  void main() {
    ...
    vec2 tc = (pp - paddedOrigin) / paddedSize;
    if (any(lessThan(tc, vec2(0.0))) || any(greaterThan(tc, vec2(1.0)))) { gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); return; }
    vec2 cp = clamp(pp, vec2(0.0), screenSize);
    float g = clamp(cp.y / screenSize.y, 0.0, 1.0);
