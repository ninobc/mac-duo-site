...
export function createStage(els) {
  ...
  // Cap the pixel ratio on big displays: a 5K canvas is wasted on a blur.
  renderer.setPixelRatio(Math.min(devicePixelRatio || 1, innerWidth > 1600 ? 1.5 : 2));
  ...
  const uniforms = {
    ...
    sheenAmount: { value: 0 }, sheenPos: { value: 0 }, grain: { value: effect.grain }, time: { value: 0 }, brightness: { value: 1 }, recede: { value: 1 }, topEdge: { value: 1e6 },
  };
  ...
  let hinge = null, rig = null;
  let displayBottomCm = 1.02, displayHeightCm = 18.43, displayWidthCm = 30.34;
  let disposed = false;
  ...
  loader.load('/models/macbook-pro-14.glb', gltf => {
    ...
    if (display) {
      ...
      displayBottomCm = b.min.y;
      displayHeightCm = b.max.y - b.min.y;
      displayWidthCm = b.max.x - b.min.x;
    }
  ...
  wallpaper.src = '/assets/wallpaper-dunes-1800.jpg';
  wallpaper.onload = () => {
    if (disposed) return;
  ...
  function update(dt, t) {
    ...
    if (reduced) { angle = angleTarget; angleVelocity = 0; } else {
      const k = 11, acc = k * k * (angleTarget - angle) - 2 * k * angleVelocity;
    ...
    // The eye is the camera, in the hinge's unrotated frame (y up, z towards
    // the viewer), measured in screen points from the display's bottom edge.
    let eye;
    if (rig) {
      rig.worldToLocal(eyeScratch.copy(camera.position));
      const ptPerCm = SCREEN.h / displayHeightCm;
      eye = {
        x: SCREEN.w / 2 + eyeScratch.x * ptPerCm,
        y: (eyeScratch.y - HINGE.y - displayBottomCm) * ptPerCm,
        z: (eyeScratch.z - HINGE.z) * ptPerCm,
      };
    }
    const f = foldFrame(effect, SCREEN.w, SCREEN.h, angle, eye);
    ...
    uniforms.recede.value = f.recede;
    uniforms.topEdge.value = f.topEdge;
    uniforms.blurFloor.value = f.blurFloor;
