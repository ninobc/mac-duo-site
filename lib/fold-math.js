...
export const presets = {
  duo:       { startAngle: 92,  span: 58, blurRadius: 125, blurFloor: 0, dimming: 1,   dimStart: 0, dimReach: 0.55, dimHingeFloor: 0.18, depth: 1,   eyeDistance: 2.4, eyeHeight: 0, sheen: 0, grain: 0 },
  soft:      { startAngle: 90,  span: 50, blurRadius: 70,  blurFloor: 0, dimming: 0.7, dimStart: 0, dimReach: 0.75, dimHingeFloor: 0.1,  depth: 0.8, eyeDistance: 3.5, eyeHeight: 0, sheen: 0, grain: 0 },
  cinematic: { startAngle: 100, span: 70, blurRadius: 160, blurFloor: 0, dimming: 1,   dimStart: 0, dimReach: 0.45, dimHingeFloor: 0.25, depth: 1.2, eyeDistance: 2.0, eyeHeight: 0, sheen: 0, grain: 0 },
};
...
export function corners(effect, W, H, angle, eye = defaultEye(effect, W, H)) {
  ...
  const separation = Math.min(effect.depth * travel, 88);
...
export function frame(effect, W, H, angle, eye) {
  const progress = Math.min(Math.max((effect.startAngle - angle) / effect.span, 0), 1);
  ...
  const recede = 1;
  // Deep into the close the hinge rows stretch across the glass; frost them too.
  const separation = Math.min(effect.depth * Math.max(effect.startAngle - angle, 0), 88);
  const blurFloor = Math.max(effect.blurFloor, smoothstep((separation - 45) / 35));
  return {
    inverse, visibleTop, progress, recede, blurFloor, topEdge: c[2][1],
    blurStrength: Math.pow(progress, 1.55),
    dimStrength: Math.pow(progress, 0.75),
